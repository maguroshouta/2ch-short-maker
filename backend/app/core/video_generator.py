...
agent = Agent(
    ...
    3つ目
    A：{テーマ}について30文字前後で述べる
    B：Aの内容は無視して25文字前後で述べる
    ...
    - 優しい語尾を使わない
    - 卑猥な言葉を使わない
    - 「{n}位」という文字列をタイトルや本文に入れない

    検索
    - 必ず1回だけ google_search(query: str) を使う
    - 検索結果は最大3件まで
    - 検索結果（markdown）を参考にしてランキング本文を書く

    キーワード
    - タイトルに合う「いらすとや」向け検索キーワードを一語で出す
    - 出力の最後に「キーワード：◯◯」の形式で書く
    """,
